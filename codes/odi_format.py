...
print('---------Running odi_format-------------------')
odi_data_path = 'data/odi/';
os.chdir(odi_data_path)

# re to match all non world cup matches.
odi_file_regex = re.compile('^matches_\d{4}\.csv$')
# re to all world cup matches
odi_wc_file_regex = re.compile('^matches_worldcup_\d{4}\.csv$')
# grab all the .csv files
odi_csv_files = glob.glob('*.csv')
...
def format_and_clean():  
    ...
    '''
    for file_name in odi_csv_files:
        
        # pick only the non world cup matches
        if odi_file_regex.match(file_name) :
            ...
            global all_odi_data
            all_odi_data.append(data_frame)
            
            # on an independent run write yearly data to disk
            if __name__ == '__main__':
                data_frame.to_csv('data/clean_format/odi/'+str(file_name), 
                                  index=False
                )
            
    
def merge_all(all_odi_data):
    """
    ...
    try:
        assert len(all_odi_data) > 0
        master_frame = pd.concat(all_odi_data, axis=0, ignore_index=True)
        global frame_row_count
        assert frame_row_count == master_frame.shape[0]
        return master_frame
    except AssertionError:
        print("Merge of dataframes failed. odi frame list incorrect.")
        raise SystemExit(1)
...
def get_wc_dates():
    ...
    try:
        ...
        for file_name in odi_csv_files:
            ...
            if odi_wc_file_regex.match(file_name) :
...
def clean_format_data():
    ...
    format_and_clean()
    global all_odi_data
    master_data = merge_all(all_odi_data)
    try:
        # on an independent run write combined data to disk
        if __name__ == '__main__':
            master_data.to_csv('data/clean_format/odi/'+'master_odi_data.csv',
                               index=False,
                              # na_rep='NA'
            )
        ...
        if __name__ == '__main__':
            clean_master.to_csv('data/clean_format/odi/'+'odi_master_clean.csv', index=False)
